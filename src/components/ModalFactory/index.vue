      ...
      >
        ...
        >
          <div class="flex flex-col px-12 py-10 bg-white">
            <component :is="state.component" v-bind="state.props" /> <!-- v-bind="state.props" passar as props -->
          </div>
        </div>
      </div>
    </div>
...
const ModalCategory = defineAsyncComponent(() => import('../ModalCategory'))

const DEFAULT_WIDTH = 'w-3/4 lg:w-1/3'

export default {
  components: { ModalLogin, modalC: ModalAccountCreate, ModalCategory },
  setup () {
    ...
    // antes de desmontar, 'desliga' o modal
    onBeforeUnmount(() => {
      modal.off(handleModalToogle)
    })

    function handleModalToogle (payload) {
      // monta componente se status for true
      if (payload.status) {
